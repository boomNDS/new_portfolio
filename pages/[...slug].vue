<script setup lang="ts">
import { stagger, useAnimate } from "motion-v";
import { computed, onMounted, onUnmounted, ref } from "vue";

// =============================================================================
// TYPES & CONSTANTS
// =============================================================================

interface ContentChild {
  tag?: string;
  children?: { value?: string }[];
}

interface RelatedArticle {
  path: string;
  title: string;
  description: string;
}

type CollectionName = "blog" | "devLogs" | "learning" | "showcases";
type UrlPath = "blog" | "dev-logs" | "learning" | "showcases";

const COLLECTION_MAP: Record<UrlPath, CollectionName> = {
  blog: "blog",
  "dev-logs": "devLogs",
  learning: "learning",
  showcases: "showcases",
};

const TYPE_CONFIG: Record<string, { label: string; color: string }> = {
  "dev-logs": { label: "Dev Log", color: "bg-blue-100 text-blue-700 border-blue-200" },
  learning: { label: "Learning", color: "bg-green-100 text-green-700 border-green-200" },
  blog: { label: "Blog", color: "bg-purple-100 text-purple-700 border-purple-200" },
  showcases: { label: "Showcase", color: "bg-orange-100 text-orange-700 border-orange-200" },
};

// =============================================================================
// ROUTE & COLLECTION SETUP
// =============================================================================

const route = useRoute();
const slug = computed(() => {
  const param = route.params.slug;
  return Array.isArray(param) ? param.join("/") : (param ?? "");
});

const urlPath = computed(() => slug.value.split("/")[0] as UrlPath);
const collectionName = computed(() => COLLECTION_MAP[urlPath.value]);
const isValidCollection = computed(() => !!collectionName.value);

// =============================================================================
// DATA FETCHING
// =============================================================================

const { data: content, pending } = await useAsyncData(
  () => `content-${slug.value}`,
  async () => {
    if (!isValidCollection.value) return null;
    try {
      return await queryCollection(collectionName.value).path(`/${slug.value}`).first();
    } catch {
      return null;
    }
  },
  { server: true, default: () => null },
);

const notFound = computed(() => !pending.value && !content.value);

// Content with optional fields for type safety
// Safe content for ContentRenderer (handles null)
const safeContent = computed(() => content.value ?? {});

// Article with optional fields for template type safety
const article = computed(() => {
  if (!content.value) return null;
  return content.value as typeof content.value & {
    difficulty?: string;
    milestone?: boolean;
    project?: string;
    progress?: number;
    prerequisites?: string[];
  };
});

// =============================================================================
// COMPUTED HELPERS
// =============================================================================

const toc = computed(() => {
  const children = (content.value?.body as { children?: ContentChild[] } | undefined)?.children;
  if (!children) return [];

  return children
    .filter((child) => child.tag === "h2")
    .map((child) => {
      const text = child.children?.[0]?.value || "";
      return {
        id: text.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
        text,
      };
    });
});

const readTime = computed(() => {
  if (!content.value?.body) return "3 min read";
  const words = JSON.stringify(content.value.body).split(" ").length;
  return `${Math.ceil(words / 200)} min read`;
});

const typeStyle = computed(
  () =>
    TYPE_CONFIG[urlPath.value] ?? {
      label: urlPath.value,
      color: "bg-gray-100 text-gray-700 border-gray-200",
    },
);

const difficultyClass = computed(
  () =>
    ({
      beginner: "bg-emerald-100 text-emerald-700 border-emerald-200",
      intermediate: "bg-amber-100 text-amber-700 border-amber-200",
      advanced: "bg-rose-100 text-rose-700 border-rose-200",
    })[article.value?.difficulty ?? ""],
);

// =============================================================================
// RELATED ARTICLES
// =============================================================================

const { data: relatedArticles } = await useAsyncData(
  () => `related-${slug.value}`,
  async () => {
    if (!isValidCollection.value || !content.value) return [];
    try {
      return await queryCollection(collectionName.value).limit(4).all();
    } catch {
      return [];
    }
  },
  { server: true, default: () => [] },
);

const relatedContent = computed(() => {
  if (!relatedArticles.value) return [];
  return (relatedArticles.value as RelatedArticle[])
    .filter((item) => item.path !== `/${slug.value}`)
    .slice(0, 3)
    .map((item) => ({
      path: item.path,
      type: urlPath.value,
      title: item.title,
      description: item.description,
    }));
});

// =============================================================================
// ANIMATIONS
// =============================================================================

const prefersReducedMotion = useMotionPreference();
const [heroScope, heroAnimate] = useAnimate();
const [contentScope, contentAnimate] = useAnimate();

onMounted(async () => {
  if (prefersReducedMotion.value === "reduce" || notFound.value) return;
  await nextTick();

  setTimeout(() => {
    heroAnimate(
      "[data-animate-hero]",
      { opacity: [0, 1], y: [30, 0] },
      { duration: 0.6, ease: [0.22, 1, 0.36, 1], delay: stagger(0.1) },
    );

    setTimeout(() => {
      contentAnimate(
        "[data-animate-content]",
        { opacity: [0, 1], y: [20, 0] },
        { duration: 0.5, ease: [0.22, 1, 0.36, 1] },
      );
    }, 300);
  }, 100);
});

// =============================================================================
// READING PROGRESS
// =============================================================================

const progress = ref(0);

onMounted(() => {
  const updateProgress = () => {
    const scrollTop = window.scrollY;
    const docHeight = document.documentElement.scrollHeight - window.innerHeight;
    progress.value = docHeight > 0 ? (scrollTop / docHeight) * 100 : 0;
  };

  window.addEventListener("scroll", updateProgress, { passive: true });
  updateProgress();

  onUnmounted(() => {
    window.removeEventListener("scroll", updateProgress);
  });
});

// =============================================================================
// SEO & UTILS
// =============================================================================

useSeoMeta({
  title: () => content.value?.title || "Article Not Found",
  description: () => content.value?.description || "The requested article could not be found.",
});

const scrollToSection = (id: string) => {
  document.getElementById(id)?.scrollIntoView({ behavior: "smooth", block: "start" });
};
</script>

<template>
  <div class="min-h-screen bg-[#fafafa] pt-16">
    <!-- 404 Not Found State -->
    <div v-if="notFound || !isValidCollection" class="min-h-[60vh] flex items-center justify-center px-4">
      <div class="text-center max-w-md">
        <span class="i-tabler:file-x text-6xl text-gray-300 mb-4 block mx-auto" />
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Article Not Found</h1>
        <p class="text-gray-600 mb-6">The article you're looking for doesn't exist or has been moved.</p>
        <NuxtLink to="/writing" class="btn-primary">
          <span class="i-tabler:arrow-left" />
          Back to Writing
        </NuxtLink>
      </div>
    </div>

    <!-- Loading State -->
    <div v-else-if="pending" class="min-h-[60vh] flex items-center justify-center">
      <div class="animate-pulse space-y-4">
        <div class="h-8 w-48 bg-gray-200 rounded" />
        <div class="h-4 w-96 bg-gray-200 rounded" />
      </div>
    </div>

    <!-- Article Content -->
    <div v-else-if="article">
      <!-- Reading Progress Bar -->
      <div class="fixed top-16 left-0 right-0 h-1 bg-gray-100 z-50">
        <div class="h-full bg-gradient-to-r from-indigo-500 to-purple-600 transition-all duration-150" :style="{ width: `${progress}%` }" />
      </div>

      <!-- Sticky Header -->
      <header class="sticky top-16 z-40 bg-white/95 backdrop-blur-sm border-b border-gray-100">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 py-3">
          <NuxtLink to="/writing" class="btn-back">
            <span class="i-tabler:arrow-left" />
            Back to Writing
          </NuxtLink>
        </div>
      </header>

      <!-- Hero Section -->
      <section ref="heroScope" class="hero-gradient border-b border-gray-100">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
          <div class="max-w-3xl space-y-6">
            <!-- Tags -->
            <div data-animate-hero class="flex flex-wrap items-center gap-2">
              <span class="badge-type" :class="typeStyle.color">{{ typeStyle.label }}</span>
              <span v-if="article.milestone" class="badge-milestone">🎯 Milestone</span>
              <span v-if="article.difficulty" class="badge-difficulty" :class="difficultyClass">
                {{ article.difficulty }}
              </span>
            </div>

            <!-- Title & Description -->
            <div data-animate-hero class="space-y-4">
              <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 tracking-tight">
                {{ article.title }}
              </h1>
              <p class="text-lg text-gray-700 leading-relaxed">{{ article.description }}</p>
            </div>

            <!-- Meta -->
            <div data-animate-hero class="flex flex-wrap items-center gap-4 text-sm text-gray-500">
              <span class="meta-item"><span class="i-tabler:calendar" />{{ formatDate(article.date) }}</span>
              <span class="meta-item"><span class="i-tabler:clock" />{{ readTime }}</span>
              <span v-if="article.project" class="meta-item">
                <span class="w-1 h-1 rounded-full bg-gray-300" />
                <span class="i-tabler:folder" />
                {{ article.project }}
              </span>
            </div>

            <!-- Progress Bar -->
            <div v-if="article.progress !== undefined" class="max-w-md space-y-2">
              <div class="flex justify-between text-sm">
                <span>Project Progress</span>
                <span class="font-medium">{{ article.progress }}%</span>
              </div>
              <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                <div class="h-full bg-indigo-500 rounded-full transition-all" :style="{ width: `${article.progress}%` }" />
              </div>
            </div>
          </div>
        </div>
      </section>

      <!-- Main Content -->
      <main class="max-w-5xl mx-auto px-4 sm:px-6 py-12">
        <div class="flex flex-col lg:flex-row gap-12">
          <!-- Article -->
          <article class="flex-1 max-w-3xl space-y-8">
            <!-- Prerequisites -->
            <div
              v-if="article.prerequisites?.length"
              class="mb-8 p-6 rounded-xl border-2 border-[var(--color-border)] bg-[var(--color-card)] shadow-[4px_4px_0px_rgba(0,0,0,0.1)]"
            >
              <h3 class="flex items-center gap-2 font-semibold text-[var(--color-dark)]">
                <span class="i-tabler:book-open text-[var(--color-primary)]" />
                Before You Start
              </h3>
              <ul class="space-y-2">
                <li v-for="prereq in article.prerequisites" :key="prereq" class="flex items-center gap-2 text-sm text-[var(--color-text)]">
                  <span class="i-tabler:check text-[var(--color-primary)]" />
                  {{ prereq }}
                </li>
              </ul>
            </div>

            <!-- TOC Mobile -->
            <nav
              v-if="toc.length"
              class="lg:hidden mb-8 p-4 rounded-xl border border-gray-200/80 bg-white/90 shadow-sm backdrop-blur-sm"
            >
              <p class="text-[11px] font-medium text-gray-400 uppercase tracking-widest mb-3">On this page</p>
              <ul class="space-y-1">
                <li v-for="item in toc" :key="item.id">
                  <button
                      type="button"
                      @click="scrollToSection(item.id)"
                      class="block w-full text-left py-1.5 text-sm text-gray-600 hover:text-[var(--color-primary)] transition-colors"
                    >
                      {{ item.text }}
                    </button>
                </li>
              </ul>
            </nav>

            <!-- Content -->
            <div ref="contentScope" data-animate-content class="prose prose-lg max-w-none">
              <ContentRenderer :value="safeContent" />
            </div>

            <!-- Tags -->
            <div v-if="content?.tags?.length" class="flex flex-wrap items-center gap-2 pt-8 border-t border-gray-100">
              <span class="text-sm text-gray-500">Tags:</span>
              <NuxtLink
                v-for="tag in content.tags"
                :key="tag"
                :to="`/writing?tag=${tag}`"
                class="px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-full text-sm font-medium hover:bg-indigo-100 transition-colors"
              >
                {{ tag }}
              </NuxtLink>
            </div>
          </article>

          <!-- TOC Desktop -->
          <aside v-if="toc.length" class="hidden lg:block lg:w-64 shrink-0">
            <div class="sticky top-24">
              <nav class="p-4 rounded-xl border border-gray-200/80 bg-white/90 shadow-sm">
                <p class="text-[11px] font-medium text-gray-400 uppercase tracking-widest mb-3">On this page</p>
                <ul class="space-y-1">
                  <li v-for="item in toc" :key="item.id">
                    <button
                      type="button"
                      @click="scrollToSection(item.id)"
                      class="block w-full text-left py-1.5 text-sm text-gray-600 hover:text-[var(--color-primary)] transition-colors"
                    >
                      {{ item.text }}
                    </button>
                  </li>
                </ul>
              </nav>
            </div>
          </aside>
        </div>
      </main>

      <!-- Related Articles -->
      <section
        v-if="relatedContent.length"
        class="border-t border-gray-100 bg-gradient-to-b from-gray-50 to-white"
      >
        <div class="max-w-5xl mx-auto px-4 sm:px-6 py-16">
          <div class="flex items-center gap-3 mb-8">
            <span class="w-8 h-8 rounded-lg bg-indigo-100 flex items-center justify-center">
              <span class="i-tabler:book-open text-indigo-600" />
            </span>
            <h2 class="text-xl font-bold text-gray-900">Continue Reading</h2>
          </div>
          <div class="grid sm:grid-cols-2 gap-6">
            <NuxtLink
              v-for="item in relatedContent"
              :key="item.path"
              :to="item.path"
              class="group flex gap-4 p-5 bg-white rounded-2xl border border-gray-100 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-100/50 transition-all duration-300"
            >
              <div class="shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-100 flex items-center justify-center">
                <span v-if="item.type === 'blog'" class="i-tabler:article text-purple-500 text-xl" />
                <span v-else-if="item.type === 'dev-logs'" class="i-tabler:code text-blue-500 text-xl" />
                <span v-else class="i-tabler:book text-green-500 text-xl" />
              </div>
              <div class="min-w-0">
                <span
                  class="inline-block px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide mb-1.5"
                  :class="TYPE_CONFIG[item.type]?.color ?? 'bg-gray-100 text-gray-700'"
                >
                  {{ TYPE_CONFIG[item.type]?.label ?? item.type }}
                </span>
                <h3 class="font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors line-clamp-1 mb-1">
                  {{ item.title }}
                </h3>
                <p class="text-sm text-gray-500 line-clamp-2">{{ item.description }}</p>
              </div>
            </NuxtLink>
          </div>
        </div>
      </section>
    </div>
  </div>
</template>

<style scoped>
.hero-gradient {
  @apply bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50;
}

.btn-primary {
  @apply inline-flex items-center gap-2 px-6 py-3 rounded-full bg-gray-900 text-white font-medium hover:bg-gray-800 transition-colors;
}

.btn-back {
  @apply inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-50 border border-gray-200 text-gray-700 font-medium hover:bg-gray-100 hover:border-gray-300 transition-all shadow-sm;
}

.badge-type {
  @apply px-2.5 py-1 rounded-full text-xs font-medium border;
}

.badge-milestone {
  @apply px-2.5 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 border border-yellow-200;
}

.badge-difficulty {
  @apply px-2.5 py-1 rounded-full text-xs font-medium border;
}

.meta-item {
  @apply flex items-center gap-1.5;
}

.card-prerequisites {
  @apply p-6 rounded-xl border-2 border-[var(--color-border)] bg-[var(--color-card)] shadow-[4px_4px_0px_rgba(0,0,0,0.1)];
}

.toc-mobile {
  @apply p-4 rounded-xl border border-gray-200/80 bg-white/90 shadow-sm backdrop-blur-sm;
}

.toc-desktop {
  @apply p-4 rounded-xl border border-gray-200/80 bg-white/90 shadow-sm;
}

.toc-title {
  @apply text-[11px] font-medium text-gray-400 uppercase tracking-widest mb-3;
}

.toc-link {
  @apply block w-full text-left py-1.5 text-sm text-gray-600 hover:text-[var(--color-primary)] transition-colors;
}

.tag-link {
  @apply px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-full text-sm font-medium hover:bg-indigo-100 transition-colors;
}

.related-section {
  @apply border-t border-gray-100 bg-gradient-to-b from-gray-50 to-white;
}

.card-related {
  @apply group flex gap-4 p-5 bg-white rounded-2xl border border-gray-100 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-100/50 transition-all duration-300;
}

.icon-wrapper {
  @apply shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-100 flex items-center justify-center;
}

.badge-related {
  @apply inline-block px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide mb-1.5;
}
</style>
